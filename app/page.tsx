...
const PRODUCTS_CACHE_KEY = 'home-products-cache';
...
export default function Home() {
  ...
  // 1. Carregamento com Cache para evitar Skeletons na volta
  useEffect(() => {
  ...
  // 2. Restauração do Scroll Instantânea e Invisível
  useLayoutEffect(() => {
    if (!loading && products.length > 0) {
      const savedScroll = sessionStorage.getItem(SCROLL_KEY);
      if (savedScroll) {
        // 1. Pula instantaneamente
        window.scrollTo({
          top: parseInt(savedScroll),
          behavior: 'auto' // 'auto' é o pulo sem animação
        });
  ...
  // 3. Função para salvar posição ao clicar
  const saveScrollPos = () => {
  ...
  return (
            ...
            {destaques.map((item) => {
              ...
              return (
                <Link
                  ...
                  className="group relative backdrop-blur-md bg-black/40 border border-white/10 hover:border-yellow-500/50 rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:shadow-yellow-500/10 hover:-translate-y-2 cursor-pointer block shadow-sm"
                >
                  ...
                  <div className="p-6 text-center">
                    <h3 className="text-lg font-medium text-gray-100 font-serif h-14 flex items-center justify-center leading-tight text-center px-2">
                      {item.title}
                    </h3>
                    ...
                    <div className="mt-1 mb-4 flex flex-col items-center gap-0.5">
                      <p className="text-xs text-gray-400 font-light">em até 12x no cartão</p>
                      <div className="flex items-center gap-1">
                        <span className="w-1 h-1 bg-yellow-500 rounded-full animate-pulse"></span>
                        <p className="text-[10px] text-yellow-500 uppercase tracking-[0.15em] font-bold">Garantia de 1 Ano</p>
                      </div>
                    </div>

                    <div className={`w-full py-3 rounded-lg text-xs font-bold uppercase tracking-widest flex items-center justify-center ${isOutOfStock
            ...
            {normais.map((item) => {
              ...
              return (
                <Link
                  ...
                  key={item.id}
                  onClick={saveScrollPos}
                  className="relative group backdrop-blur-sm bg-black/40 border border-white/10 hover:border-yellow-500/50 rounded-xl p-4 transition-all duration-300 hover:shadow-xl hover:-translate-y-2 cursor-pointer block shadow-sm"
                >
                  ...
                  <div className="px-1 flex flex-col items-center text-center w-full">
                    <h4 className="text-[13px] md:text-sm text-gray-200 font-medium group-hover:text-yellow-400 transition-colors h-[60px] flex items-center justify-center leading-tight line-clamp-3 text-center px-1">
                      {item.title}
                    </h4>
    ...
    <div className="flex flex-col items-center gap-2 mt-2 mb-1">
        <p className="text-[8px] md:text-[9px] text-green-400 font-bold bg-green-900/20 px-2 py-0.5 rounded-full border border-green-500/20 tracking-widest uppercase">
            Disponível
        </p>
        <p className="text-[8px] md:text-[9px] text-yellow-500 uppercase tracking-[0.2em] font-black drop-shadow-[0_0_3px_rgba(234,179,8,0.3)]">
    Garantia de 1 Ano
</p>
    </div>
)}

                    <div className={`w-full mt-3 text-[10px] border border-white/20 py-2 rounded-md transition-all uppercase font-bold hover:shadow-md text-center ${isOutOfStock ? 'text-gray-600 cursor-not-allowed' : 'text-gray-300 group-hover:bg-yellow-500 group-hover:text-black group-hover:border-yellow-500'
                      }`}>
