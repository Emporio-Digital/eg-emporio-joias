...
export default function ProdutoDetalhe({ params }: { params: Promise<{ id: string }> }) {
    ...
    const handleAddToCart = () => {
        ...
        if (produto.sizes?.length > 0 && !selectedSize) {
            setSizeError(true);
            const el = document.getElementById('tamanhos-area');
                            ...
                            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                                {todasImagens.map((img, idx) => (
                                    <button
                                        ...
                                        className={`relative w-20 h-20 flex-shrink-0 rounded-lg overflow-hidden border-2 transition-all ${activeImage === img
                    ...
                    <div className="w-full md:w-1/2 flex flex-col animate-fade-in bg-neutral-900/80 backdrop-blur-md p-6 md:p-10 rounded-3xl border border-white/10 shadow-xl">
                        ...
                        <div className="mb-8 flex flex-col gap-2">
                            <p className="text-sm text-gray-300">
                                Em até 12x no cartão ou <span className="text-green-400 font-bold">5% OFF</span> no Pix Direto.
                            </p>
                            <div className="flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/20 w-fit px-3 py-1.5 rounded-full">
                                ...
                                <span className="text-[10px] text-yellow-500 uppercase tracking-[0.2em] font-black">Garantia de 1 Ano</span>
                            </div>
                        </div>
                        ...
                        {/* SELEÇÃO DE TAMANHOS */}
                        <div className="mb-10" id="tamanhos-area">
                            <h3 className={`text-sm font-bold uppercase tracking-widest mb-3 transition-all ${sizeError ? 'text-red-500 animate-pulse' : 'text-white'}`}>
                                {categoriaNormalizada === 'colar' ? 'Comprimento' : categoriaNormalizada === 'pulseira' ? 'Tamanho' : 'Selecione a Opção'}
                                {sizeError && <span className="ml-2 text-[10px] bg-red-500 text-white px-2 py-0.5 rounded animate-bounce">Obrigatório</span>}
                            </h3>

                            <div className={`flex gap-3 flex-wrap p-2 rounded-2xl transition-all ${sizeError ? 'bg-red-500/10 ring-2 ring-red-500/40' : ''}`}>
                                {produto.sizes && produto.sizes.length > 0 ? (
                                    produto.sizes.map((t: string) => (
                                        <button
                                            ...
                                            className={`btn-tamanho px-4 w-auto transition-all duration-300 touch-manipulation font-black ${
                                                selectedSize === t 
                                                ? '!bg-yellow-500 !text-black border-yellow-400 shadow-[0_0_25px_rgba(234,179,8,0.8)] scale-110 z-10' 
                                                : 'bg-neutral-800 text-gray-400 border-white/10 hover:border-yellow-500/50'
                                            }`}
                                        >
                                            {t}
                                        </button>
                            ...
                            {guiaAtual && (
                                <button
                                    ...
                                    className="group mt-6 flex items-center gap-2 px-4 py-2 rounded-lg border border-yellow-500/20 bg-yellow-500/5 transition-all hover:bg-yellow-500/10 hover:border-yellow-500/40"
                                >
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3.5 h-3.5 text-yellow-500 transition-transform group-hover:scale-110">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                                    </svg>
                                    <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-yellow-500/90 group-hover:text-yellow-500">
                                        Guia de Medidas <span className="text-gray-400 group-hover:text-gray-300 ml-1 font-normal uppercase tracking-normal italic"></span>
                                    </span>
                                </button>
                            )}
                        </div>
                        ...
                        <div className="flex flex-col gap-4">
                            <button
                                ...
                                className={`w-full py-4 text-sm font-bold uppercase tracking-[0.2em] rounded-xl transition-all transform hover:-translate-y-1 active:scale-95 ${isOutOfStock
